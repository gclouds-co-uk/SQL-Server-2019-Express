...
locals {
  ...
  metadata = {
    ...
    sql-patch = "cu32"
    release = "2026H2"
    source-os-image = "windows-server-2019-dc-v20260429"
...
resource "google_compute_firewall" tcp_3389 {
  count = var.enable_tcp_3389 ? 1 : 0

  name = "${var.goog_cm_deployment_name}-tcp-3389"
  ...
  source_ranges =  compact([for range in split(",", var.tcp_3389_source_ranges) : trimspace(range)])
